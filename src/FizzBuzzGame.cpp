...
FizzBuzzGame::FizzBuzzGame(int first, int second, int third) {
	...
	Handler* _m1 = new Handler(_default, new MultipleMatcher(first),
			new Action(FIZZ));
	_handlers.push_back(_m1);
	Handler* _m2 = new Handler(_m1, new MultipleMatcher(second),
			new Action(BUZZ));
	_handlers.push_back(_m2);
	Handler* _m3 = new Handler(_m2, new MultipleMatcher(third),
			new Action(WHIZZ));
